...
assert.equal(candidate.published, true, "The prior r23 release remains published while r24 is prepared");
assert.equal(candidate.chsDraftConfigurationUpdated, verifiedSaved, "The r24 saved flag may be true only after its CHS save is verified");
assert.equal(candidate.chsSubmissionStatus, "not_submitted");
assert.equal(candidate.publishedOn, "2026-09-18");
assert.equal(candidate.lastPublishedRelease, verifiedSaved ? "chs-home-school-evelyn-v1-r24-yellow-cleanup-1" : "chs-home-school-evelyn-v1-r23-two-role-sets-1");
assert.equal(candidate.candidateRelease, "chs-home-school-evelyn-v1-r24-yellow-cleanup-1");
assert.equal(candidate.revisionPendingPublication, !verifiedSaved);
assert.equal(candidate.chsDraftRelease, verifiedSaved ? "chs-home-school-evelyn-v1-r24-yellow-cleanup-1" : "chs-home-school-evelyn-v1-r23-two-role-sets-1");
assert.equal(candidate.latestChsDraftSaveReceipt, verifiedSaved ? "review/chs-draft-save-r24.md" : "review/chs-draft-save-r23.md");
...
assert.match(app, /const HOME_SCHOOL_ASSET_VERSION = "chs-home-school-evelyn-v1-r24-yellow-cleanup-1"/);
...
assert.match(html, /chs-home-school-evelyn-v1-r24-yellow-cleanup-1/g);
