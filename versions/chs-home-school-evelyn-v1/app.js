...
const HOME_SCHOOL_ASSET_VERSION = "chs-home-school-evelyn-v1-r11";
const HOME_SCHOOL_DESIGN_VERSION = "home_school_context_chs_candidate_v1";
const HOME_SCHOOL_CONTEXT_SCRIPT_VERSION = "home_school_context_recipient_aware_v3";
...
const EVENT_MANIFEST_URL = runtimeConfig.eventManifestUrl
  || `data/ksize_manifest.json?v=${HOME_SCHOOL_ASSET_VERSION}`;
...
const MISSING_HOME_SCHOOL_AUDIO_MANIFEST_URL = runtimeConfig.missingHomeSchoolAudioManifestUrl
  || `data/missing_home_school_directional_audio_manifest.json?v=${HOME_SCHOOL_ASSET_VERSION}`;
...
const DATA_MIRROR_CONTRACT = "find-the-caregiver-sheets-v1";
...
let dataMirrorAnswerRowsShared = 0;
...
function makeDataPayload(jsPsych) {
  ...
  return {
    ...
    data_mirror_contract: DATA_MIRROR_CONTRACT,
    data_mirror_mode: shouldMirrorAnswerRowsToParent
      ? "parent-answer-rows-plus-final"
      : (requestedDataEndpoint ? "direct-final" : "off"),
    data_mirror_answer_rows_shared: dataMirrorAnswerRowsShared,
...
function isAnswerRowForDataMirror(row) {
  if (!row || typeof row !== "object") return false;
  return row.slide_kind === "response_choices"
    || row.slide_kind === "response"
    || row.choice_index != null
    || row.rating_value != null;
}

function shareAnswerRowWithParent(jsPsych, row) {
  if (!shouldMirrorAnswerRowsToParent || !isAnswerRowForDataMirror(row)) return;
  if (!window.parent || window.parent === window) return;
  const allRows = jsPsych.data.get().values();
  const mirrorRowIndex = Math.max(0, allRows.length - 1);
  dataMirrorAnswerRowsShared += 1;
  const snapshot = makeDataPayload(jsPsych);
  const { rows: _rows, ...sessionMetadata } = snapshot;
  const message = {
    type: "DATA_MIRROR_ROW",
    ...sessionMetadata,
    completed_at: "",
    ...
    mirror_row_index: mirrorRowIndex,
    row,
  };
...
async function handleStudyFinish(jsPsych) {
  ...
  const completionMessage = {
      ...
      data_post_status: result.reason,
...
function introImageForSlide(trial, slide, slideIndex) {
  const sourceImage = slide.images[0];
  const fixedImage = localImage(introImageFixes[`${trial.id}|${slideIndex}`]);
  if (!fixedImage) return sourceImage;
  return {
    ...fixedImage,
    homeSchoolForegroundSrc: sourceImage?.homeSchoolForegroundSrc || null,
  };
}
...
function recipientKeyForCondition(condition) {
  return ({
    "DAD-KID": "MOM",
    "MOM-KID": "DAD",
    "TEACHER-KID": "TEACHER",
  })[String(condition || "").toUpperCase()] || "KID";
}

function contextEventSpec(eventSuffix, context = activeStudyContext, condition = "") {
  const spec = contextSpec(context);
  const recipientKey = recipientKeyForCondition(condition);
  return spec?.recipientEvents?.[recipientKey]?.[eventSuffix]
    || spec?.events?.[eventSuffix]
    || null;
}

function contextualizedEventLines(lines, eventSuffix, context = activeStudyContext, condition = "") {
  const replacements = contextEventSpec(eventSuffix, context, condition);
...
function contextAudioIdForText(text) {
  ...
  const contextTexts = [
    ...
    ...Object.values(spec?.recipientEvents || {}).flatMap((eventGroup) =>
      Object.values(eventGroup || {}).flatMap((event) => [event.eventText, event.questionText])
    ),
  ].filter(Boolean);
...
function activeContextEventSpec(condition = "") {
  return activeStudyContext && activeStudyEvent
    ? contextEventSpec(activeStudyEvent, activeStudyContext, condition)
    : null;
}
...
function missingContextAudioForPlan(eventPlan, eventSuffix, context = activeStudyContext) {
  ...
  for (const trial of eventPlan || []) {
    ...
    if (!block) continue;
    const baseLines = String(block.text || "").split(/\n+/).map((line) => line.trim()).filter(Boolean);
    const condition = block.condition || trial?.blocks?.INTRO?.condition || "";
    requiredLines.push(...contextualizedEventLines(baseLines, eventSuffix, context, condition).slice(0, 2));
  }
  return [...new Set(requiredLines.filter(Boolean))].filter((line) => !canonicalAudioPathForText(line));
}
...
function assertParticipantContextAudioCoverage(eventPlan, eventSuffix, context = activeStudyContext) {
  ...
  throw new Error(
    `This Home/School candidate is not ready for participant collection: ${missing.length} `
    + `approved Evelyn narration clip${missing.length === 1 ? " is" : "s are"} still missing `
    + `for the assigned ${context.toLowerCase()} condition. Recording list: ${manifestUrl}`
  );
}
...
function renderKidSlide({ trial = null, image, text, choices = [], overlayChoices = false, showText = false, slideKind = "", showNext = false, visualChoices = false, contextIntro = false, contextCondition = "", storyNumber = null, storyTotal = null }) {
  ...
  const imageBlock = overlayChoices && image
    ...
    : (image
      ? `<div class="ksize-slide-image${contextOverlayText ? " ksize-contextual-stimulus" : ""}${furnishedScene ? " ksize-furnished-scene" : ""}"${furnishedScene ? ` data-palette="${escapeHtml(furnishedScene.paletteSlug)}" data-visual-version="${escapeHtml(furnishedScene.visualVersion)}"` : ""}>
          ${furnishedScene
...
async function main() {
  ...
  const jsPsych = initJsPsych({
    on_data_update: (data) => {
      shareAnswerRowWithParent(jsPsych, data);
      if (!isFacilitatorMode || facilitatorStudyFinished) return;
      window.setTimeout(() => {
